...
fun! ConditionalPrettier()
  ...
  let ft_string = &ft . ''
  if ft_string =~? 'handlebars'
    echo "Gonna prettier"
    silent %!prettier --parser html --stdin-filepath % --trailing-comma all --single-quote<CR>
    echo "diddy"
  else
    Prettier
  endif
...
autocmd BufWritePre *.js,*.jsx,*.mjs,*.ts,*.tsx,*.css,*.less,*.scss,*.json,*.graphql,*.md,*.mdx,*.html,*.json5 call ConditionalPrettier()
...
autocmd FileType rust let b:noNeomake=1
autocmd FileType typescript let b:noNeomake=1
...
nnoremap <Leader>i :CocAction<CR>
